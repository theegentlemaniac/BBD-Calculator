...
function toggleNegative() {
    ...
    if (negativePattern.test(expressionText)) {
        expressionText = expressionText.replace(negativePattern, function (_match, captured) { return captured; });
        updateDisplays();
        return;
    }
    if (positivePattern.test(expressionText)) {
        expressionText = expressionText.replace(positivePattern, function (_match, captured) { return "(-" + captured + ")"; });
        updateDisplays();
        return;
    }
    expressionText += "(-";
...
document.querySelectorAll(".btn").forEach(btn => {
    btn.addEventListener("click", () => {
        appendCharacter(btn.dataset.num || "");
    });
});
...
document.querySelectorAll(".sci").forEach(btn => {
    btn.addEventListener("click", () => {
        handleScientific(btn.dataset.fn || "");
    });
});
